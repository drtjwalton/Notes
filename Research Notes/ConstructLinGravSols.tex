...
\usepackage[colorlinks=true, pdfborder={0 0 0}]{hyperref}
	\hypersetup{	linkcolor=[rgb]{0.50, 0, 0.125},
					citecolor=[rgb]{0,0,1}}
...
\titleformat{\section}[block]
    ...
    {\thesection}{0.5em}
    {}[{\addvspace{0.6ex}\titlerule[1pt]\addvspace{1ex}{\normalsize\OrnamentDiamondSolid}}]
...
\newenvironment{prop}[1]{
	...
	\def\@currentlabel{\thesection\hspace{-0.10cm}--\arabic{thmcounter}}
    \def\FrameCommand
    {%
...
{	\vspace{-0.6cm}
    ...
    {%
        ...
        \fboxsep=\FrameSep\fcolorbox{black}{white}%
    }%
	\MakeFramed{\hsize\hsize\advance\hsize-\width\FrameRestore}%
	{\bf Proof:}
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section we outline a procedure to construct solutions to (\ref{LIN_EIN}) from a complex scalar field $\alpha$. Since the solutions are to be established on a flat Minkowski spacetime endowed with metric $\etam$, the curvature operator is identically zero: $R^{(\etas)}_{X,Y}=0$. Consequently, using (\ref{LapFunc}) and (\ref{LapRComm}), the rough Laplace operator and connection acting upon $\alpha$ or on any $1$-form $\omega\in\Gamma\Lambda^{1}\M$ commute:
\begin{align*}
	\LapR^{(\etas)}(\nabla^{(\etas)} \alpha) &\,=\,\nabla^{(\etas)}\left[\LapR^{(\etas)}(\alpha)\right] \QUAD{and}
	\LapR^{(\etas)}(\nabla^{(\etas)} \omega) \,=\, \nabla^{(\etas)}\left[\LapR^{(\etas)}(\omega)\right].
\end{align*}
Repeated application of this result gives:
\begin{align}\label{nabnabLap}
	\nabla^{(\etas)}\nabla^{(\etas)}\left[\LapR^{(\etas)}(\alpha)\right] &\,=\, \nabla^{(\etas)}\LapR^{(\etas)}(\nabla^{(\etas)}\alpha) \,=\, \LapR^{(\etas)}(\nabla^{(\etas)}\nabla^{(\etas)}\alpha) \,=\, \LapR^{(\etas)}(\nabla^{(\etas)}d\alpha)
\end{align}
...
\begin{align*}
	\LapR^{(\etas)}(\alpha) &\,=\, 0 \qLRA \Tpsi \QUAD{is also traceless and} \LapR^{(\etas)}(\Tpsi) \,=\, 0.
\end{align*}
Furthermore
\begin{align*}
	\Div^{(\etas)}(\Tpsi) &\,=\, \Div^{(\etas)}(\nabla^{(\etas)}d\alpha) \,=\, \LapR^{(\etas)}(d\alpha) \,=\, d\left[ \LapR^{(\etas)}(\alpha)\right]
\end{align*}
...
\newpage
\appendix
\addtocontents{toc}{\protect\vskip1cm}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Laplace Operators Acting on a 1-form}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This proposition can be proven directly using a number of the relations given in the preceding section. \\
...
\begin{proof}
	...
	\begin{align}
		...
		\nonumber &\,=\,  \LapR(\omega) - e^{b} \w i_{X^{a}}R_{X_{a},X_{b}}\,\omega \\
			\label{Weit3} &\hspace{1.8cm}	 + e^{b} \w \left[i_{X^{a}}\nabla_{\nabla_{X_{b}}X_{a}}\omega+ (\nabla_{X_{a}}\omega)(\nabla_{X_{b}}X^{a})\right]
	\end{align}
	using the definition of the curvature operator (\ref{CurvOp}). This can be further simplified by noting that
	\begin{align*}
		i_{X^{a}}R_{X_{a},X_{b}}\,\omega &\,=\, (\,R_{X_{a},X_{b}}\,\omega \,)(X^{a}) \,=\, e^{a}(\,R_{X_{a},X_{b}}\,\wt{\omega} \,) \,=\, \texttt{R}(X_{a},X_{b},\wt{\omega},e^{a}) \\[0.2cm]
		&\,=\, \texttt{R}(\wt{\omega},\wt{e^{a}},X_{a},\wt{X_{b}})  \,=\, \texttt{R}(\wt{\omega},X^{a},X_{a},e_{b}) \,=\, e_{b}( \,R_{\,\wt{\omega},X^{a}}\,X_{a} \,)  \\[0.2cm]
		&\,=\, (\, R_{\,\wt{\omega},X^{a}}\,\wt{X_{a}}\, )(\wt{e_{b}}) \,=\, ( R_{\,\wt{\omega},X^{a}}\,e_{a}\, )(X_{b})  \,=\, i_{X_{b}}\,R_{\,\wt{\omega},X^{a}}\,e_{a} \,=\, i_{X_{b}}\,R_{\,\wt{\omega},X_{a}}\,e^{a}
	\end{align*}
...
This result follows trivially from the definitions in appendix~\ref{sect:notation}:\\[0.2cm]
...
As with the previous propositions, we proceed using the definitions and identities outlined in appendix~\ref{sect:notation}:\\[0.2cm]
